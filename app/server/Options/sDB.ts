...
class Database {
  ...
  private connectDatabase (self: Database = this): void {
  ...
  private async try (query: string): Promise<any> {
    return await new Promise((resolve, reject) => this.connection.query(query, null, (err: any, data: any) => {
      if (err) {
        Logger.error(query)
        return reject(err)
      }
  ...
  private async createTables (): Promise<void> {
    ...
    for (const table of tables) {
      const result = await this.query(`SHOW tables FROM \`${this.config.database}\` LIKE '${table.name}';`)

      if (!result[0]) {
        Logger.warn(`'${table.name}' table not found! Trying to create...`)

        for (const query of table.queries) {
          Logger.warn(query)
