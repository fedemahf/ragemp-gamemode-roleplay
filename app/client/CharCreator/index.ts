...
class CharCreator {
  private readonly localPlayer = mp.players.local
  private currentGender = 0
  private readonly creatorMenus = []
  ...
  // CREATOR APPEARANCE
  private readonly appearanceItems = []
  private readonly appearanceOpacityItems = []
  private readonly opacities = []

  // CREATOR FEATURES
  private readonly featureItems = []
  private readonly features = []

  // CREATOR PARENTS
  private readonly similarities = []
  ...
  // color arrays
  private readonly hairColors = []
  private readonly blushColors = []
  private readonly lipstickColors = []
  ...
  constructor () {
    ...
    // CREATOR MAIN
    const creatorMainMenu = new NativeUI.Menu('Creator', '', new NativeUI.Point(50, 50))
    const genderItem = new NativeUI.UIMenuListItem('Gender', '~r~Changing this will reset your customization.', new NativeUI.ItemsCollection(['Male', 'Female']))
    ...
    const angleItem = new NativeUI.UIMenuListItem('Angle', '', new NativeUI.ItemsCollection(angles))
    creatorMainMenu.AddItem(angleItem)

    const saveItem = new NativeUI.UIMenuItem('Save', 'Save all changes.')
    ...
    const cancelItem = new NativeUI.UIMenuItem('Cancel', 'Discard all changes.')
    ...
    for (let i = 0; i < Data.featureNames.length; i++) {
      const tempFeatureItem = new NativeUI.UIMenuListItem(Data.featureNames[i], '', new NativeUI.ItemsCollection(this.features))
    ...
    for (let i = 0; i < Data.appearanceNames.length; i++) {
      ...
      const tempAppearanceItem = new NativeUI.UIMenuListItem(Data.appearanceNames[i], '', new NativeUI.ItemsCollection(items))
      this.appearanceItems.push(tempAppearanceItem)
      this.creatorAppearanceMenu.AddItem(tempAppearanceItem)

      const tempAppearanceOpacityItem = new NativeUI.UIMenuListItem(Data.appearanceNames[i] + ' Opacity', '', new NativeUI.ItemsCollection(this.opacities))
    ...
    this.creatorAppearanceMenu.ListChange.on((item, listIndex) => {
      const idx = (this.creatorAppearanceMenu.CurrentSelection % 2 === 0) ? (this.creatorAppearanceMenu.CurrentSelection / 2) : Math.floor(this.creatorAppearanceMenu.CurrentSelection / 2)
    ...
    mp.events.add('toggleCreator', (active, charData) => {
      if (active) {
        ...
        if (charData) {
          ...
          // hair and colors
          const hair = Data.hairList[this.currentGender].find(h => h.ID === charData.Hair.Hair)
  ...
  private colorForOverlayIdx (index): number {
    let color: number = 0
  ...
  private updateHairAndColors (): void {
    this.localPlayer.setComponentVariation(2, Data.hairList[this.currentGender][this.hairItem.Index].ID, 0, 2)
  ...
  private applyCreatorOutfit (): void {
  ...
  private resetParentsMenu (refresh = false): void {
  ...
  private resetFeaturesMenu (refresh = false): void {
    for (let i = 0; i < Data.featureNames.length; i++) {
  ...
  private resetAppearanceMenu (refresh = false): void {
    for (let i = 0; i < Data.appearanceNames.length; i++) {
  ...
  private resetHairAndColorsMenu (refresh = false): void {
