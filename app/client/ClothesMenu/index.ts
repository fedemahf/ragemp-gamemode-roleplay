...
class ClothesMenu {
  private readonly listClothesMenuItem: ClothesMenuItem[] = [
  ...
  constructor () {
    ...
    for (const clothesMenuItem of this.listClothesMenuItem) {
  ...
  private createMenuItem (clothesMenuItem: ClothesMenuItem): void {
    const itemsDrawable: string[] = []
    const itemsTextureArray: string[] = []
    ...
    const itemMenuList = new UIMenuListItem(clothesMenuItem.itemTitle, clothesMenuItem.itemDescription, new ItemsCollection(itemsDrawable), drawableId)
    const itemMenuTextureList = new UIMenuListItem(clothesMenuItem.itemVariationTitle, clothesMenuItem.itemVariationDescription, new ItemsCollection(itemsTextureArray), (clothesMenuItem.isProp ? this.player.getPropTextureIndex(clothesMenuItem.componentOrPropId) : this.player.getTextureVariation(clothesMenuItem.componentOrPropId)))
    menuItem.AddItem(itemMenuList)
    menuItem.AddItem(itemMenuTextureList)
    menuItem.Visible = false

    menuItem.ListChange.on((item, listIndex) => {
      const drawable: number = parseInt(itemMenuList.SelectedItem.DisplayText)
      const texture: number = parseInt(itemMenuTextureList.SelectedItem.DisplayText)
      const callFunction: string = (clothesMenuItem.isProp ? 'setProp' : 'setClothes')

      switch (item) {
        case itemMenuList: {
          mp.events.callRemote(callFunction, clothesMenuItem.componentOrPropId, drawable, 0)
          const itemsTextureNewArray = []
